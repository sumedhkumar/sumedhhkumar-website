...
// Parse: custom-solutions sends FormData
$fullName                = sanitize($_POST['fullName'] ?? '');
$emailAddress            = sanitize($_POST['emailAddress'] ?? '');
$phoneOrWhatsapp         = sanitize($_POST['phoneOrWhatsapp'] ?? '');
$companyOrOrganization   = sanitize($_POST['companyOrOrganization'] ?? '');
$solutionType            = sanitize($_POST['solutionType'] ?? '');
$requirementsDescription = sanitize($_POST['requirementsDescription'] ?? '');
$preferredTimeline       = sanitize($_POST['preferredTimeline'] ?? '');
$sourcePage              = sanitize($_POST['sourcePage'] ?? 'Homepage Custom Solutions Section');
...
if (strlen($requirementsDescription) < 10)        $errors['requirementsDescription'] = 'Describe your requirements.';
if (strlen($requirementsDescription) > 5000)      $errors['requirementsDescription'] = 'Keep your description within 5000 characters.';
...
$istTs      = strtotime($timestamp) + (int)(5.5 * 3600);
...
if ($supabaseUrl && $supabaseKey) {
    ...
    if ($dbHttpCode < 200 || $dbHttpCode >= 300) {
        error_log("custom-solutions.php DB insert failed ($dbHttpCode): $dbResult");
    }
}
...
function smtpSend($host, $port, $user, $pass, $from, $fromName, $to, $subject, $textBody, $htmlBody, $replyTo = '', $bcc = []) {
    ...
    if (!$sock) {
        ...
        return mail($allTo, $subject, $htmlBody, $simpleHeaders) ? '' : "mail() failed";
    }
...
if (!$smtpUser || !$smtpPass) {
    echo json_encode(['ok' => true, 'message' => 'Your requirement has been submitted. Vyntegra will review it and respond within 24 hours.']);
    exit;
}
...
if ($companyOrOrganization) $adminDetails[] = ['label' => 'Company / Organization', 'value' => $companyOrOrganization];
if ($solutionType)          $adminDetails[] = ['label' => 'Type of Solution',       'value' => $solutionType];
if ($preferredTimeline)     $adminDetails[] = ['label' => 'Expected Timeline',      'value' => $preferredTimeline];
$adminDetails[] = ['label' => 'Source Page',  'value' => $sourcePage];
$adminDetails[] = ['label' => 'Submitted At', 'value' => $istDisplay];

$adminText = implode("\n\n", array_merge(
    ["A new custom solution requirement has been submitted.", "Full Name: $fullName", "Email: $emailAddress"],
    ...
    ["Requirement:", $requirementsDescription, "Source Page: $sourcePage", "Submitted At: $istDisplay"]
));

$adminHtml = buildEmailHtml(
    'New custom solution requirement submitted',
    'A new custom solution requirement has been submitted.',
    $adminDetails,
    [['heading' => 'Requirement', 'body' => $requirementsDescription]]
);
...
if ($smtpErr1) error_log("custom-solutions.php admin email error: $smtpErr1");

$custDetails = [];
if ($solutionType)          $custDetails[] = ['label' => 'Solution Type',           'value' => $solutionType];
if ($companyOrOrganization) $custDetails[] = ['label' => 'Company / Organization',  'value' => $companyOrOrganization];
if ($preferredTimeline)     $custDetails[] = ['label' => 'Preferred Timeline',      'value' => $preferredTimeline];

$custText = implode("\n\n", array_merge(
    ["Hi $fullName,", "We have received your custom solution requirement."],
    $solutionType          ? ["Solution Type: $solutionType"] : [],
    $companyOrOrganization ? ["Company / Organization: $companyOrOrganization"] : [],
    $preferredTimeline     ? ["Preferred Timeline: $preferredTimeline"] : [],
    ["Your submitted requirement:", $requirementsDescription, "Our team will review the details and get back to you within 24 hours.", "Regards,", "Vyntegra"]
));

$custHtml = buildEmailHtml(
    'Vyntegra custom solution requirement received',
    "Hi $fullName, we have received your custom solution requirement.",
    $custDetails,
    [
...
if ($smtpErr2) error_log("custom-solutions.php customer email error: $smtpErr2");
